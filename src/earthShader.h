...
void main() {
    ...
    vec3 continentColor = vec3(0.702,0.871,0.243);  // Green for continents
    ...
    result = createGround(vec2(0.0, 1.0), 0.3, baseColor, shadedColor);
    result = createGround(vec2(-0.9, -1.0), 0.1, result, shadedColor);
    ...
    result = createGround(vec2(0.5, 4.0), 2.0, result, vec3(1.0));

    // Create a "ground" in the form of the south pole
    vec3 poleShadedColor1 = lightColor * (ambientStrength * ambientColor + diffuseStrength * poleColor);
    // Creating the South Pole
    result = createGround(vec2(0.0, -1.0), 2.0, result, vec3(1.0));
